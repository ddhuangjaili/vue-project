<template>
  <div class="clock" :style="{'height': (height - 6) + 'px'}">
    <!-- <canvas id="cnv"></canvas> -->
    <img src="/static/index.png" style="width: 100%;">
  </div>
</template>
<script>
  export default {
    ...
    computed: {
      companyId() {
        ...
        if (loginUser && loginUser.data && loginUser.data.companyId) {
          return loginUser.data.companyId;
        }
        return '';
      },
      firstPath() {
        return this.$store.getters.firstPath;
      }
    },
    mounted() {
      // if (this.companyId) {
      //   if (!this.defaultMap[this.companyId]) {
      //     clock();
      //   } else if (this.firstPath) {
      //     location.href = '#' + this.firstPath;
      //   }
      // } else {
      //   clock();
      // }
    ...
    watch: {
      firstPath(newValue, oldValue) {
        if (newValue && this.companyId && this.defaultMap[this.companyId]) {
          setTimeout(()=> {
  ...
  function drawFace(ctx) {
    ...
    g2.addColorStop(0, 'rgb(72, 87, 106)');
    g2.addColorStop(1, '#03b3b2');
