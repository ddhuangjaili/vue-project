<template>
  <div>
    ...
    <el-dialog :title="dialog.title" v-model="dialog.show" size="tiny" :close-on-press-escape="false" :close-on-click-modal="false">
      <el-form ref="seatGroupForm" :model="dialog.form" :rules="dialog.rules" label-width="120px" class="dialog-form">
        <el-row :gutter="20">
          <el-col :span="24">
            <el-form-item label="客服组代码：" prop="code">
        ...
        <el-row :gutter="20">
          <el-col :span="24">
            <el-form-item label="客服组名称：" prop="name">
...
<script>
    ...
    computed: {
      optAuth() {
        return bsmp.util.mapAuth({
          search: 'crm_service_seatGroup_view',
          add: 'crm_service_seatGroup_add',
          edit: 'crm_service_seatGroup_edit',
          del: 'crm_service_seatGroup_delete'
        });
      }
    },
    methods: {
      ...
      showDialog (type, index) {
        ...
        if (this.$refs['seatGroupForm'] && this.$refs['seatGroupForm'].resetFields) {
          this.$refs['seatGroupForm'].resetFields();
        }
      ...
      submitForm () {
        this.$refs['seatGroupForm'].validate((valid) => {
          if (valid) {
            if (this.dialog.type === 1) {
              // 新增
              api.addSeatGroup(this.dialog.form).then((res) => {
                let data = res.data;
                if (data && data.code === '0') {
                  this.$message({
                    type: 'success',
                  ...
                  bsmp.log({
                    ...
                    type: 'add',
                    content: `[SeatGroup] params: ${JSON.stringify(this.dialog.form)}`
                  });
                }
              });
            } else {
              // 修改
              api.editSeatGroup(this.dialog.form).then((res) => {
                let data = res.data;
                if (data && data.code === '0') {
                  this.$message({
                    type: 'success',
      ...
      deleteItem (id) {
        this.$confirm('确定要删除该条记录?', '提示', {
          ...
          // 删除
          api.removeSeatGroup(id).then((res) => {
            let data = res.data;
            if (data && data.code === '0') {
              this.$message({
                type: 'success',
              ...
              bsmp.log({
                component: 'crm_service',
                type: 'delete',
                content: `[SeatGroup] params: ${JSON.stringify({id})}`
              });
            }
          });
        }).catch(e => {
      ...
      query () {
        ...
        this.loading = true;
        api.getSeatGroupList(params).then((res) => {
          this.loading = false;
          let data = res.data;
          if (data && data.code === '0') {
  ...
  let rules = {
    code: [
        {required: true, message: '请输入客服组代码', trigger: 'blur'}
    ],
    name: [
      {required: true, message: '请输入客服组名称', trigger: 'blur'}
    ]
  };
</script>
