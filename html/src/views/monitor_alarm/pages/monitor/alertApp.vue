<template>
  <div>
    ...
    <el-dialog :title="dialog.title" v-model="dialog.show" :close-on-press-escape="false" :close-on-click-modal="false">
      <el-form ref="alertAppForm" :model="dialog.form" :rules="dialog.rules" label-width="140px" class="dialog-form">
        <el-row :gutter="20">
          <el-col :span="12">
  ...
  export default {
    ...
    computed: {
      optAuth() {
        return bsmp.util.mapAuth({
          search: 'monitor_alarm_alert_app_list',
          save: 'monitor_alarm_alert_app_save',
          del: 'monitor_alarm_alert_app_delete'
        });
      },
      company() {
    ...
    methods: {
      ...
      showDialog (type, index) {
        if (this.$refs['alertAppForm'] && this.$refs['alertAppForm'].resetFields) {
          this.$refs['alertAppForm'].resetFields();
        }
      ...
      submitForm () {
        this.$refs['alertAppForm'].validate((valid) => {
          if (valid) {
            if (this.dialog.type === 1) {
              ...
              api.saveAlertApp(this.dialog.form).then((res) => {
                let data = res.data;
                if (data && data.code === '0') {
                  this.$message({
                    type: 'success',
                  ...
                  bsmp.log({
                    ...
                    type: 'add',
                    content: `[AlertApp] params: ${JSON.stringify(this.dialog.form)}`
                  });
                }
              });
            } else {
              // 修改
              api.updateAlertApp(this.dialog.form).then((res) => {
                let data = res.data;
                if (data && data.code === '0') {
                  this.$message({
                    type: 'success',
      ...
      deleteItem (id) {
        this.$confirm('确定要删除该条记录?', '提示', {
          ...
          // 删除
          api.removeAlertApp(id).then((res) => {
            let data = res.data;
            if (data && data.code === '0') {
              this.$message({
                type: 'success',
              ...
              bsmp.log({
                component: 'monitor_alarm',
                type: 'delete',
                content: `[AlertApp] params: ${JSON.stringify({id: id})}`
              });
            }
          });
        }).catch(e => {
      ...
      query () {
        ...
        api.getAlertAppList(params).then((res) => {
          this.loading = false;
          let data = res.data;
          if (data && data.code === '0') {
