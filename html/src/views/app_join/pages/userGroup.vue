  ...
  <div>
    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <el-button @click="showMessageDialog(scope.row)" size="mini"><i class="icon-chat-empty"></i>&nbsp;&nbsp;推送消息
          </el-button>
          <el-button @click="showUserDialog(scope.row)" size="mini"><i class="icon-users"></i>&nbsp;&nbsp;用户列表
          </el-button>
	        <el-button title="删除" type="danger" icon='delete' size="mini" @click="deleteItem(scope.row.tag)"></el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="dialog.title" v-model="dialog.show" size="tiny" :close-on-press-escape="false" :close-on-click-modal="false">
      <el-form ref="tagForm" :model="dialog.form" :rules="dialog.rules" label-width="120px" class="dialog-form">
        <el-row :gutter="20">
          <el-col :span="24">
            <el-form-item label="组名称：" prop="tag">
    ...
    <!-- 弹窗框 -->
    <el-dialog title="组用户列表" v-model="userDialog.show" size="tiny" :close-on-press-escape="false" :close-on-click-modal="false">
      <el-row class="toolbar">
  ...
  export default {
    ...
    methods: {
      ...
      sendMsg () {
        ...
        api.sendTagMsg(this.table.appCode, this.message.tag.tag, {msg: this.message.text}).then((res) => {
          let data = res.data;
          if (data && data.code === '0') {
