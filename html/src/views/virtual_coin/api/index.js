...
const path = '/api/virtual_coin/admin/';
...
export default {
  ...
  getChatMsgList(params) {
    ...
    let companyId=JSON.parse(user).data.companyId;
    return axios({method: 'get', url: companyId!='vc2c'?`/api/virtual_coin_msg_${companyId}/admin/msg`:`/api/virtual_coin_msg/admin/msg`, params: params});
  },
  getFormatPath(uri) {
    let user = window.sessionStorage.getItem('user');
    let companyId=JSON.parse(user).data.companyId;
    if(companyId!='vc2c'){
        return '/api/virtual_coin_'+companyId+'/admin/'+uri;
    }
    return path+uri;
  },
   /**
  ...
  modifyNameCn(params) {
    return axios({method: 'post', url: this.getFormatPath('client/modifyNickname'), data: qs.stringify(params)});
  },
  unbindIdCard(params) {
    return axios({method: 'post', url: this.getFormatPath('client/unbindIdCard'), data: qs.stringify(params)});
  },
  toBindWalletId(params) {
    return axios({method: 'post', url: this.getFormatPath('client/toBindWalletId'), data: qs.stringify(params)});
  },
   ...
   */
  getSmsList (params){
    return axios({method: 'get', url: this.getFormatPath('smsInfo'), params: params});
  },
   ...
   * @returns {AxiosPromise}
   */
  getChargedProposalList (params) {
    return axios({method: 'get', url: this.getFormatPath('chargedProposal'), params: params});
  },
  editChargedProposal (params) {
    return axios({method: 'put', url: this.getFormatPath(`chargedProposal/${params.id}`), data: qs.stringify(params)});
  },
  executeWalletNotice (params) {
    return axios({method: 'put', url: this.getFormatPath(`chargedProposal/executeWalletNotice/${params.id}`), data: qs.stringify(params)});
  },
  deleteChargedProposal (id) {
    return axios({method: 'delete', url: this.getFormatPath(`chargedProposal/${id}`)});
  },
  deleteChargedProposals (id) {
    return axios({method: 'delete', url: this.getFormatPath(`chargedProposal/deleteByIds/${id}`)});
  },
};
