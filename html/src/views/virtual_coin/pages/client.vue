...
<script>
  ...
  export default {
    ...
    computed: {
      optAuth() {
        return bsmp.util.mapAuth({
          ...
          search: 'virtual_coin_client_view',
          ...
          export: 'virtual_coin_client_export',
          tradeInit: 'virtual_coin_client_trade_init',
          modifyNameCn: 'virtual_coin_client_edit_nickname',
          unbindIdcard: 'virtual_coin_client_unbindIdcard',
          bindWallet:'virtual_coin_client_bindWallet',
        });
      },
      hasBankCard() {
    ...
    methods: {
      ...
      videoAuthFmt(row){
        if(row.accountList && row.accountList.length>0){
            let vt=row.accountList[0].videoAuth;
            return vt==2?'已认证':'未认证';
         }
         return '未认证';
      },
      ...
      getClientByMobileNo(mobileNo) {
        ...
        api.getClientByMobileNo(mobileNo).then((res) => {
          ...
          if (data && data.code === '0') {
            ...
            if(form.accountList){
                this.dialog.form.videoAuth =form.accountList[0].videoAuth;
