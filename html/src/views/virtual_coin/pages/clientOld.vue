  ...
  <div>
        ...
        <template slot-scope="props">
          <table class="stat-table" v-if="props.row.accountList && props.row.accountList.length > 0">
            ...
            <tr v-for="(account, key) in props.row.accountList">
              ...
              <td>
                <div v-for="(accountCur, i) in account.accountCurList">
    ...
    <el-dialog title="充币" v-model="fundsDialog.show" :close-on-press-escape="false" :close-on-click-modal="false">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="submitFundsForm" type="primary" :disabled="fundsDialog.form.availableAmount < 1 || fundsDialog.form.amount < 1">保 存</el-button>
            ...
            <el-row :gutter="20">
              <el-col :span="12">
                <el-form-item label="姓名：">
                  <el-input v-model="dialog.form.unifyInfo.baseInfo.cnName" auto-complete="off" :disabled="true"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row :gutter="20">
            ...
            <el-row :gutter="20">
              <el-col :span="12">
                <el-form-item label="电邮：">
                  <el-input v-model="dialog.form.unifyInfo.baseInfo.email" auto-complete="off" :disabled="true"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row :gutter="20">
              ...
              <el-col :span="12">
                <el-form-item label="证件号码：">
                  <el-input v-model="dialog.form.unifyInfo.idCardInfo.idCardNo" auto-complete="off" :disabled="true"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
          </el-tab-pane>
              ...
              <tr>
                ...
                <td>
                  <el-upload
                    ...
                    :on-success="(res, file, files) => {avatarSuccess(res, file, files, 'back', 'idCard')}"
                    :before-upload="(file) => {beforeUpload(file, 'back', 'idCard')}"
                    :on-error="(err, file, fileList) => {avatarError(err, file, fileList, 'back', 'idCard')}"
                    :data="uploadFile.uploadFileData">
                    <el-button :loadiing="imagesMap.front.uploading"
                               :disabled="this.dialog.form.advancedAuth === '1'"
              ...
              <tr>
                ...
                <td>
                  <el-upload
                    :action="uploadFile.uploadFileUrl"
                    :show-file-list="false"
                    :on-success="(res, file, files) => {avatarSuccess(res, file, files, 'front', 'idCard')}"
                    :before-upload="(file) => {beforeUpload(file, 'front', 'idCard')}"
                    :on-error="(err, file, fileList) => {avatarError(err, file, fileList, 'front', 'idCard')}"
                    :data="uploadFile.uploadFileData">
                    <el-button :loadiing="imagesMap.front.uploading"
                               :disabled="this.dialog.form.advancedAuth === '1'"
                               type="primary" size="small">
              ...
              <tr>
                ...
                <td>
                  <el-upload
                    :action="uploadFile.uploadFileUrl"
                    :show-file-list="false"
                    :on-success="(res, file, files) => {avatarSuccess(res, file, files, 'holdon', 'idCard')}"
                    :before-upload="(file) => {beforeUpload(file, 'holdon', 'idCard')}"
                    :on-error="(err, file, fileList) => {avatarError(err, file, fileList, 'holdon', 'idCard')}"
                    :data="uploadFile.uploadFileData">
                    <el-button :loadiing="imagesMap.front.uploading"
                               :disabled="this.dialog.form.advancedAuth === '1'"
                               type="primary" size="small">
...
<script>
  ...
  export default {
    ...
    computed: {
      optAuth() {
        return bsmp.util.mapAuth({
          //add: 'virtual_coin_client_add',
          search: 'virtual_coin_client_old_view',
          //edit: 'virtual_coin_client_edit',
          //auth: 'virtual_coin_client_auth',
          //charge: 'virtual_coin_client_charge',
          //clientStatus: 'virtual_coin_client_update_client_status',
          //clientGroup: 'virtual_coin_client_update_client_group',
          //accountStatus: 'virtual_coin_client_update_account_status',
          //accountFlag: 'virtual_coin_client_update_account_flag',
          //authCodeList: 'virtual_coin_client_auth_code_list',
          //uploadConfig: 'virtual_coin_client_upload_config',
          //openAccount: 'virtual_coin_client_open_account',
          //categoryAuth:'virtual_coin_client_category_auth',
          export: 'virtual_coin_client_old_export',
          //tradeInit: 'virtual_coin_client_trade_init'
        });
      },
      hasBankCard() {
    ...
    methods: {
      ...
      avatarSuccess(res, file, files, fileType, businessType) {
      ...
      avatarError(err, file, fileList, fileType, businessType) {
        this.imagesMap[fileType].uploading = false;
      ...
      showClientGroupDialog(clientCode, tags) {
        ...
        this.clientGroupDialog.form.tags = tags + '';
      ...
      getAvailableAmount() {
        ...
        this.accounts && this.accounts.length && this.accounts.forEach((account) => {
          account.accountCurList && account.accountCurList.length && account.accountCurList.forEach((item) => {
            if (item.currencyCode === this.fundsDialog.form.currencyCode) {
      ...
      submitForm() {
        ...
        if (this.dialog.form.unifyInfo.baseInfo.email
          && !bsmp.util.isEmail(this.dialog.form.unifyInfo.baseInfo.email)) {
        ...
        let params = {
          ...
          email: this.dialog.form.unifyInfo.baseInfo.email,
        ...
        api.getClientList(params).then((res) => {
          ...
          if (data && data.code === '0') {
            if (data.data.data instanceof Array) {
              ...
              let filedHeader = ['客户呢称', '客户编码', '手机号码', '状态','客户组别', '真实姓名',
                  '实名认证', '身份认证', '创建时间'],
                fieldNames = ['nickName', 'clientCode', 'mobileNo', 'status','tags', 'realName',
                  'realNameAuth', 'advancedAuth', 'createDate'];
