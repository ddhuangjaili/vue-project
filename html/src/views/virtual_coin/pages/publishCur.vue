    ...
    computed: {
      optAuth() {
        return bsmp.util.mapAuth({
          search: 'virtual_coin_publishCur_view',
          add: 'virtual_coin_publishCur_add',
          edit: 'virtual_coin_publishCur_edit',
          del: 'virtual_coin_publishCur_del',
          batch: 'virtual_coin_publishCur_batch'
        });
      }
    },
    methods: {
       ...
       */
      getCurrencyList () {
        api.getCurrencyList({pageNo: 1, pageSize: 1000}).then((res) => {
          let data = res.data;
          if (data && data.code === '0') {
