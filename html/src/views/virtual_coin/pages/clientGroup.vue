  ...
  <div>
    <query-panel>
      <el-form :inline="true" :model="search" class="demo-form-inline">
        <el-form-item>
          <el-input v-model="search.name" placeholder="组别名称"></el-input>
        </el-form-item>
        <el-form-item>
    ...
    <el-dialog :title="dialog.title" v-model="dialog.show" :close-on-press-escape="false" :close-on-click-modal="false">
      <el-form ref="clientGroupForm" :model="dialog.form" :rules="dialog.rules" label-width="120px" class="dialog-form">
        <el-row :gutter="20">
          <el-col :span="12">
    ...
    computed: {
      optAuth() {
        return bsmp.util.mapAuth({
          search: 'virtual_coin_client_group_view',
          add: 'virtual_coin_client_group_add',
          edit: 'virtual_coin_client_group_edit',
          del: 'virtual_coin_client_group_del'
        });
      }
    },
    methods: {
      ...
      showDialog (type, index) {
        if (this.$refs['clientGroupForm'] && this.$refs['clientGroupForm'].resetFields) {
          this.$refs['clientGroupForm'].resetFields();
        }
        ...
        this.$refs['clientGroupForm'].validate((valid) => {
          if (valid) {
            ...
            if (this.dialog.type === 1) {
              // 新增
              api.addClientGroup(params).then((res) => {
                let data = res.data;
                if (data && data.code === '0') {
                  this.$message({
                    type: 'success',
                  ...
                  bsmp.log({
                    ...
                    type: 'add',
                    content: `[clientGroup] params: ${JSON.stringify(params)}`
                  });
                }
              });
            } else {
              ...
              // 修改
              api.editClientGroup(params).then((res) => {
                let data = res.data;
                if (data && data.code === '0') {
                  this.$message({
                    type: 'success',
      ...
      deleteItem (id, type) {
        this.$confirm(type === 1 ? '确定要删除该条记录?': '确定要删除所选记录?', '提示', {
          ...
          let method = api.deleteClientGroup(id);
          ...
          method.then((res) => {
            ...
            if (data && data.code === '0') {
              ...
              bsmp.log({
                component: 'virtual_coin',
                type: 'delete',
                content: `[clientGroup] params: ${JSON.stringify({id: id})}`
              });
            }
          });
        }).catch(e => {
      ...
      query () {
        ...
        api.getClientGroupList(params).then((res) => {
          this.loading = false;
          let data = res.data;
          if (data && data.code === '0') {
