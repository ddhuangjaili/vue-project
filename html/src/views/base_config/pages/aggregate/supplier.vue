  ...
  <div>
    <query-panel>
      <el-form :inline="true" :model="search" class="demo-form-inline">
        ...
        <el-form-item>
          <el-select v-model="search.infCode" clearable filterable placeholder="调用接口编号">
            <el-option v-for="(val, key) in infCodeMap" :label="val" :value="key"
                       :key="'search-infCode-'+ key"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item>
    ...
    <el-dialog :title="dialog.title" v-model="dialog.show" :close-on-press-escape="false" :close-on-click-modal="false">
      <el-form ref="mailConfForm" :model="dialog.form" :rules="dialog.rules" label-width="140px" class="dialog-form">
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="供应商：" prop="supplier">
              <el-input v-model="dialog.form.supplier" placeholder="请输入供应商" auto-complete="off"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="请求方法：" prop="method">
              <el-select v-model="dialog.form.method" placeholder="请求方法">
        ...
        <el-row :gutter="20">
          <el-col :span="24">
            <el-form-item label="调用接口编号：" prop="infCode">
              <el-input v-model="dialog.form.infCode" auto-complete="off" placeholder="如输入mobile_auth和idcard_auth分别表示手机认证和身份证认证"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="已使用的次数：" prop="usedTimes">
              <el-input-number v-model="dialog.form.usedTimes" :min="0"></el-input-number>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="总次数：" prop="totalTimes">
              <el-input-number v-model="dialog.form.totalTimes" :min="0"></el-input-number>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="20">
          <el-col :span="24">
            <el-form-item label="供应商apiUrl：" prop="apiUrl">
        ...
        <el-row :gutter="20">
          <el-col :span="24">
            <el-form-item label="验证通过返回：" prop="pass">
        ...
        <el-row :gutter="20">
          <el-col :span="24">
            <el-form-item label="验证不通过返回：" prop="notPass">
...
<script>
  ...
  export default {
    name: 'base_config_aggregate_supplier',
    props: {
      height: {
        type: Number
      }
    ...
    computed: {
      optAuth() {
        return bsmp.util.mapAuth({
          search: 'base_config_aggregate_supplier_list',
          save: 'base_config_aggregate_supplier_save',
          del: 'base_config_aggregate_supplier_delete'
        });
      },
      company() {
    ...
    methods: {
       ...
       */
      infCodeFmt (row) {
        return typeof row.infCode !== 'undefined' ? this.infCodeMap[row.infCode] : '';
      },
      /**
       * 显示弹窗
       * */
        ...
        this.$refs['mailConfForm'].validate((valid) => {
          if (valid) {
            if (this.dialog.type === 1) {
              ...
              api.saveAggregateSupplier(this.dialog.form).then((res) => {
                let data = res.data;
                if (data && data.code === '0') {
                  this.$message({
                    type: 'success',
                  ...
                  bsmp.log({
                    ...
                    type: 'add',
                    content: `[AggregateSupplier] params: ${JSON.stringify(this.dialog.form)}`
                  });
                }
              });
            } else {
      ...
      deleteItem (id) {
        this.$confirm('确定要删除该条记录?', '提示', {
          ...
          // 删除
          api.removeAggregateSupplier(id).then((res) => {
            let data = res.data;
            if (data && data.code === '0') {
              this.$message({
                type: 'success',
              ...
              bsmp.log({
                component: 'base_config',
                type: 'delete',
                content: `[AggregateSupplier] params: ${JSON.stringify({id: id})}`
              });
            }
          });
        }).catch(e => {
      ...
      query () {
        ...
        api.getAggregateSupplierList(params).then((res) => {
          this.loading = false;
          let data = res.data;
          if (data && data.code === '0') {
  ...
  let rules = {
    ...
    infCode: [
      {required: true, message: '请输入调用接口编号', trigger: 'blur'}
    ]
  };
</script>
