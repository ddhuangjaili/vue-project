    ...
    computed: {
      optAuth() {
        return bsmp.util.mapAuth({
          search: 'ms_center_admin_platformGroup_view',
          status: 'ms_center_admin_platformGroup_status'
        });
      }
    },
    methods: {
      ...
      statusItem(code, status) {
        ...
        this.$confirm(title, '提示', {
          ...
          // 删除
          api.adminPlatformGroupStatus(this.table.appCode, code, status).then((res) => {
            let data = res.data;
            if (data && data.code === '0') {
              this.$message({
                type: 'success',
              ...
              bsmp.log({
                component: 'ms_center',
                type: 'modify',
                content: `[AdminPlatformGroup] params: ${JSON.stringify({code, status})}`
              });
            }
          });
        }).catch(e => {
