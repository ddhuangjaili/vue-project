    ...
    computed: {
      optAuth() {
        return bsmp.util.mapAuth({
          search: 'vc_gz_currency_report_view',
          export: 'vc_gz_currency_report_export',
        });
      }
    },
    methods: {
      ...
      query () {
        ...
        api.getCurrencyReportList(params).then((res) => {
          ...
          let lists = data.data;//列表数据
          if (lists && lists.code === '0') {
            this.table.items = lists.data.data;
            this.table.pager = Object.assign({}, this.table.pager,
              {
                pageNo: parseInt(lists.data.pageNo, 10),
                total: lists.data.total
              });
              this.table.footer = data.amountSum;
          }
        });
      },
      ...
      getSum(param) {
        ...
        const sums = [];
        let buyValues = [], sallValues = [], chargeValues = [], withdrawValues = [], outValues = [], inValues = [];
            data.forEach((row) => {
                if(row.type == 0) {
                    buyValues.push(row.amount);
                }
                if(row.type == 1) {
                    sallValues.push(row.amount);
                }
                if(row.type == 2) {
                    chargeValues.push(row.amount);
                }
                if(row.type == 3) {
                    withdrawValues.push(row.amount);
                }
                if(row.type == 4) {
                    inValues.push(row.amount);
                }
                if(row.type == 5) {
                    outValues.push(row.amount);
                }
            });
            columns.forEach((column, index) => {
                ...
                if(index === 1){
                    ...
                    },0)+'\n总计：'+this.table.footer.buyerTotal;
                }
                if(index === 2){
                    sums[index] = '卖出\n小计：'+sallValues.reduce((prev, curr) => {
                        const value = Number(curr);
                        if(!isNaN(value)){
                            return prev + curr;
                        }else {
                    ...
                    }, 0)+'\n总计：'+this.table.footer.sallerTotal;
                }
                if(index === 3){
                    sums[index] = '充币\n小计：'+chargeValues.reduce((prev, curr) => {
                        const value = Number(curr);
                        if(!isNaN(value)){
                            return prev + curr;
                        }else {
                    ...
                    }, 0)+'\n总计：'+this.table.footer.chargeTotal;
                }
                if(index === 4){
                    sums[index] = '提币\n小计：'+withdrawValues.reduce((prev, curr) => {
                        const value = Number(curr);
                        if(!isNaN(value)){
                            return prev + curr;
                        }else {
                    ...
                    }, 0)+'\n总计：'+this.table.footer.withdrawTotal;
                }
                if(index === 5){
                    sums[index] = '后台转入\n小计：'+inValues.reduce((prev, curr) => {
                        const value = Number(curr);
                        if(!isNaN(value)){
                            return prev + curr;
                        }else {
                    ...
                    }, 0)+'\n总计：'+this.table.footer.intoTotal;
                }
                if(index === 6){
                    sums[index] = '后台转出\n小计：'+outValues.reduce((prev, curr) => {
                        const value = Number(curr);
                        if(!isNaN(value)){
                            return prev + curr;
                        }else {
