...
export default {
 ...
 getChatMsgList(params) {
    let companyId=JSON.parse(window.sessionStorage.getItem('user')).data.companyId;
    return axios({method: 'get', url: `/api/vc_gz_msg_${companyId}/admin/msg`, params: params});
  },
  getFormatPath(uri) {
    let companyId=JSON.parse(window.sessionStorage.getItem('user')).data.companyId;
    return '/api/vc_gz_'+companyId+'/admin/'+uri;
  },
   /**
   * 客户信息
   * @param params
